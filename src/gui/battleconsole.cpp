...
void BattleConsole::printAttacks(const std::vector<std::string> & vec, unsigned int highlight) {
	...
	Console::moveCursorToCol(static_cast<int>(Console::getWidth() / 3 - vec[0].size() / 2));
	...
	if (vec.size() > 1) {
		Console::moveCursorToCol(static_cast<int>(Console::getWidth() * 2 / 3 - vec[1].size() / 2));
	...
	if (vec.size() > 2) {
		Console::moveCursorDown(2);
		Console::moveCursorToCol(static_cast<int>(Console::getWidth() / 3 - vec[2].size() / 2));
	...
	if (vec.size() > 3) {
		Console::moveCursorToCol(static_cast<int>(Console::getWidth() * 2 / 3 - vec[3].size() / 2));
...
void BattleConsole::printOwn(const std::string & name, unsigned int level, float health,
	...
	const auto num1 = Console::getWidth() * 2 / 3 - 4 - static_cast<unsigned int>(line.size() + lineEnd.size());
	const auto num2 = Console::getMinWidth() - 4 - static_cast<unsigned int>(line.size() + lineEnd.size());
	instance().m_bars = std::max(num1, num2);
	auto healthBars = static_cast<unsigned int>(std::ceil(health / static_cast<float>(maxHealth)
		* static_cast<float>(instance().m_bars)));
	...
	const auto bars = instance().m_bars / 2;
	healthBars = static_cast<unsigned int>(std::floor(static_cast<float>(XP)
		/ static_cast<float>(maxXP) * static_cast<float>(bars)));
...
void BattleConsole::printFoe(const std::string & name, unsigned int level, float health, unsigned int maxHealth) {
	...
	std::string line(name + " (Level " + std::to_string(level) + ")");
	Console::moveCursorToCol(static_cast<int>(Console::getWidth() - line.size() - 2));
	wprintw(Console::win(), line.c_str());
	...
	const auto healthBars = static_cast<unsigned int>(std::ceil(health / static_cast<float>(maxHealth)
		* static_cast<float>(instance().m_bars)));

	Console::moveCursorToCol(static_cast<int>(Console::getWidth() - line.size() - lineEnd.size() - instance().m_bars - 2));
	waddstr(Console::win(), line.c_str());
...
void BattleConsole::printText() const {
	...
	for (unsigned int i = 0; i < m_textHeight; ++i) {
		...
		if (m_text.size() > i) {
			...
			Console::moveCursorRight(static_cast<int>((Console::getWidth() - 2 - m_text[i].size()) / 2));
